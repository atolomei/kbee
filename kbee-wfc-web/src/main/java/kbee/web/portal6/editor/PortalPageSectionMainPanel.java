...
public class PortalPageSectionMainPanel extends PortalPanel<PageSection> {

	private static final long serialVersionUID = 1L;
	
	private AjaxTabbedPanel<ITab> tabbed_panel;
	
	public PortalPageSectionMainPanel(String id, IModel<PageSection> model) {
		super(id, model);
	...
	public void onInitialize() {
		...
		add(title);
		AjaxLink<PageSection> close = new AjaxLink<PageSection>("close", PortalPageSectionMainPanel.this.getModel()) {
			
			private static final long serialVersionUID = 1L;
			
			@Override
			public void onClick(AjaxRequestTarget target) {
				fireScanAll (new PortalCloseEditAjaxEvent<PageSection>(target, PortalPageSectionMainPanel.this.getModel()));
			}
		};
		add(close);		
		...
		tabs.add(new AbstractTab(new StringResourceModel("control", this, null)) {
		        ...
		        public Panel getPanel(String panelId) {
		        	return new PortalPageSectionEditor(panelId, getModel());
		        }
		});
