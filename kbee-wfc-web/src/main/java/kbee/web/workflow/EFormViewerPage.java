 ...
 *  <p>
 *  eForm Audit -> 
 *  web page without js links, only eform
 *  This page must be able to render a eform with the css they were created, even if later versions of the applications have updated the css
 *  
 *  
 *  </p>
 *  
 *  From Process History it should be a static html
 *   
 *   
 */
public class EFormViewerPage extends KbeeWebPage<Void> {
	...
	public EFormViewerPage(PageParameters param) {
		super();
		this.activitymodel	= new ObjectModel<Activity>(getActivity(param));
		this.datamodel	= new ObjectModel<EFormData>(getEFormData(param));
		setName( (datamodel!=null && datamodel.getObject()!=null)? datamodel.getObject().getForm().getDisplayName():"");
	}
	
	public EFormViewerPage(IModel<Activity> activitymodel,  IModel<EFormData> datamodel) {
	...
	public void onInitialize() {
		...
		left_panel.add(new EFormCaptureViewerPanel("eform-audit", datamodel));

		if (this.datamodel.getObject().isSigned()) {
			left_panel.add(
					new ESignatureViewer("signature-panel", this.datamodel)
					);
		}
		else
			left_panel.add(new InvisiblePanel("signature-panel"));
			
		
		//left_panel.add(new EFormViewerPanel("eform-audit", datamodel));
		

		
		right_panel.add(new  AuditEFormSidePanel("activity-panel", this.activitymodel, this.datamodel));
	...
	protected KbeeWorkflowActivity getActivity() {
		return (KbeeWorkflowActivity)activitymodel.getObject();
	}
	...
	private EFormData getEFormData(PageParameters param) {
		StringValue ename = param.get("ename");
		if (ename==null) return null;
		for (EFormData data : getActivity().getFormsData()) {
			if (data.getForm().getName().equals(ename.toString())) {
				return data;
			}	
		}	 
		return null;
