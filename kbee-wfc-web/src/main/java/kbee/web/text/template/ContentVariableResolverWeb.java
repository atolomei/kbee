package kbee.web.text.template;
...
public class ContentVariableResolverWeb<T extends Content> implements VariableResolver {

	static final Properties properties = PropertiesFactory.getInstance("kbee").getProperties();


	private static kbee.util.logging.Logger logger = kbee.util.logging.Logger.getLogger(ContentVariableResolverWeb.class.getName());
	
	private ContentEditor<T> xxeditor;
	private T content;
	
	public ContentVariableResolverWeb(ContentEditor<T> editor) {
		this.xxeditor = editor;
	}
	
	public ContentVariableResolverWeb(T content) {
	...
	public T getContent() {
		return content!=null ? content : getEditor().getModelObject();
	}
	
	public ContentEditor<T> getEditor() {
		return xxeditor;
	}
	...
	protected List<Classification> getClassification() {
		return  getEditor()!=null ? getEditor().getClassification() : getContent().getClassification();
	}
	
	protected List<String> getAttributeValue(Attribute attribute) {
		List<String> values = getEditor()!=null ? getEditor().getAttributeValue(attribute) : getContent().getAttributeValues(attribute);
		return values;
	}
