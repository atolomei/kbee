...
public class EAutoCompletePanel<T> extends EFieldPanel<EAutoCompleteField<T>> {
	private static final long serialVersionUID = 1L;
	
	
	public EAutoCompletePanel(String id, EAutoCompleteField<T> field, IModel<EFormData> data) {
		super(id, field, data);
	}
		...
		getContainer().add(
			new AutoCompleteFieldV5<T>("field", 	new FieldDataModel<EAutoCompleteField<T>, T>(getFieldModel(), getDataModel())) {
				...
				public void onUpdate(AjaxRequestTarget target) {
					...
					updateModel();
					EAutoCompletePanel.this.onUpdate(target);
				}
				@Override
				...
				public boolean hasFeedback() {
					return !EAutoCompletePanel.this.getMessages().isEmpty();
				}
				@Override
				public String getMessage() {
					return hasFeedback() ? EAutoCompletePanel.this.getMessages().get(0).toString() : null;
				}
				@Override
				...
				public Disposition getDisposition() {
					return EAutoCompletePanel.this.getDisposition();
				}
				@Override
				...
				protected String getValue(Suggestion suggestion) {
					...
					return object != null 
						? EAutoCompletePanel.this.getValue(object)
						: suggestion.getText();
				}
				@Override
				@SuppressWarnings("unchecked")
				protected String getInfo(Suggestion suggestion) {
					...
					return suggestion.getObject() != null 
						? EAutoCompletePanel.this.getInfo(object)
						: suggestion.getText();
				}
		...
		menu.addItem(id ->
			new AjaxMenuItemPanelV5<Void>(id) {
				...
				public void onClick(AjaxRequestTarget target) {
					EAutoCompletePanel.this.refresh(target);
				}	
				@Override
				public String getLabel() {	
					return EAutoCompletePanel.this.getLabelString("refresh");
				}
		});
