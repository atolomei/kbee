...
public class EDateTimePanel extends EFieldPanel<KbeeEDateTimeField> {
	private static final long serialVersionUID = 1L;
	
	public EDateTimePanel(String id, KbeeEDateTimeField field, IModel<EFormData> data) {
		super(id, field, data);
	}
	...
	public void onInitialize() {
		...
		getContainer().add(new DateTimeField("field", 
				getZoneId(), 
				new FieldDataModel<KbeeEDateTimeField, OffsetDateTime>(getFieldModel(), getDataModel())) {
			@Override
			...
			public void onUpdate(AjaxRequestTarget target) {
				...
				updateModel();
				EDateTimePanel.this.onUpdate(target);
			}
			@Override
			...
			public boolean hasFeedback() {
				return !EDateTimePanel.this.getMessages().isEmpty();
			}
			@Override
			...
			public String getMessage() {
				return hasFeedback() ? EDateTimePanel.this.getMessages().get(0).toString() : null;
			}
			@Override
			public Disposition getDisposition() {
				return EDateTimePanel.this.getDisposition();
			}
			@Override
		...
		getInput().addInfoPanel((id) -> new EMetainfoPanel("panel") {
			...
			public List<Serializable> getMessages() {
				return EDateTimePanel.this.getMessages();
			}
		});
