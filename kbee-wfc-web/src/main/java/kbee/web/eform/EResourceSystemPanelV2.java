...
public class EResourceSystemPanelV2 extends EFieldPanel<KbeeEResourceSystemV2> implements ResourcesPanel, IFormModelUpdateListener {
	private static final long serialVersionUID = 1L;
	
	private static Logger logger = Logger.getLogger(EResourceSystemPanelV2.class.getName());
	...
	//private Map<String, ResourceTreeNode> nodes = new HashMap<>();
	...
	public class TreeFragment extends Fragment {
		...
		public TreeFragment(String id) {
			super(id, "tree-fragment", EResourceSystemPanelV2.this);
				...
				protected Component newContentComponent(String id, IModel<ResourceTreeNode> node) {
					...
					folder.add(new DroppableBehavior() {
						protected void onDrop(AjaxRequestTarget target, String id) {
							...
							IModel<ResourceNode> droppedmodel = EResourceSystemPanelV2.this.getResource(id);
	...
	public class ToolbarFragment extends Fragment {
		public ToolbarFragment(String id) {
			super(id, "toolbar-fragment", EResourceSystemPanelV2.this);
			...
			selectAll.add(new CheckField("selectAll", new PropertyModel<Boolean>(EResourceSystemPanelV2.this, "selectAll")) {
				@Override
			...
			sortchoices.setChoiceRenderer(new ChoiceRenderer<Sort>() {
				...
				public String getDisplayValue(Sort value) {
					return EResourceSystemPanelV2.this.getLabelString(value.toString().toLowerCase());
				};
			});
		...
		public void setSort(Sort sort) {
			EResourceSystemPanelV2.this.sort = sort;
		}
		
		public Sort getSort() {
			return EResourceSystemPanelV2.this.sort;
		}
		...
		protected void setBreadcrumb() {
			...
			bc.addElement(new AjaxBCElement<ResourceNode>(new Model<String>(getField().getLabel()), null) {
				public void onClick(AjaxRequestTarget target) {
					setFolder(null);
					EResourceSystemPanelV2.this.fireScanAll(new TreeNodeSelection<ResourceTreeNode>(target, null));
				}
			});
			if (foldermodel!=null)
			for (IModel<ResourceNode> nodemodel : getPathNodes(foldermodel)) {
				bc.addElement(new AjaxBCElement<ResourceNode>(nodemodel, new Model<String>(nodemodel.getObject().getDisplayName())) {
					public void onClick(AjaxRequestTarget target) {
						...
						EResourceSystemPanelV2.this.fireScanAll(new TreeNodeSelection<ResourceTreeNode>(target, new Model<ResourceTreeNode>(node)));
					}
				});
			}
			addOrReplace(bc);
		...
		protected Panel getMenu() {
			try {
				...
				menu.addItem(id ->
					new AjaxCheckMenuItemPanelV5<KbeeEResourceSystemV2>(id) {
						@Override
				...
				menu.addItem(id ->
					new SeparatorMenuItemPanelV5<KbeeEResourceSystemV2>(id) {
						@Override
				...
				menu.addItem(new MenuItemFactory<KbeeEResourceSystemV2>() {
					@Override
					public AbstractMenuItemPanelV5<KbeeEResourceSystemV2> getItem(String id) {
						return new com.novamens.wicket.markup.html.actions.AjaxCheckMenuItemPanelV5<KbeeEResourceSystemV2>(id) {
							@Override
							public void onCheckClick(AjaxRequestTarget target) {
								try {
									// DashboardContentWidgetPanel.this.onViewMode(target, "compact");
	...
	public class ListFragment extends Fragment {
		...
		public ListFragment(String id) {
			super(id, "list-fragment", EResourceSystemPanelV2.this);
	...
	public class SelectionFragment extends Fragment {
		public SelectionFragment(String id) {
			super(id, "selection-fragment", EResourceSystemPanelV2.this);
			setOutputMarkupId(true);
		}
		public void onBeforeRender() {
			super.onBeforeRender();
	...
	public class ActionFragment extends Fragment {
		public ActionFragment(String id) {
			super(id, "action-fragment", EResourceSystemPanelV2.this);
	...
	public class ControlFragment extends Fragment {
		public ControlFragment(String id) {
			super(id, "control-fragment", EResourceSystemPanelV2.this);
			...
			WebMarkupContainer selection  = new WebMarkupContainer("selection-view") {
				public boolean isVisible() {
					return isSelectionVisible();
				}
			};
			
			selection.add(getSelectionPanel());
			...
			WebMarkupContainer pickfiles = new WebMarkupContainer("pickfiles") {
				public boolean isVisible() {
					return isEditionEnabled() && !isReadOnly() && EResourceSystemPanelV2.this.isEnabled() && !isQuotaLimit() && !isSelectionVisible();
				}
			};
			add(pickfiles);
	...
	public class ResourceView extends Fragment {
		...
		public ResourceView(IModel<Resource> model) {
			super("resource-view", "resource-view-fragment", EResourceSystemPanelV2.this);
			setModel(model);
			setOutputMarkupId(true);
			add(getSelector());
			add(getView(model));
			...
			add(new ResourceEditor("editor", model, getContentModel()) {
				...
				public void onUpdate(AjaxRequestTarget target) {
					...
					updated = true;
					setUpdatedField(new ResourceUpdated(getData().getForm(), EResourceSystemPanelV2.this.getLabel(), getModelObject()));
					target.add(ResourceView.this);
					EResourceSystemPanelV2.this.onUpdate(target);
				}
				@Override
		...
		public void setSelected(Boolean value) {
			if (value)
				EResourceSystemPanelV2.this.select(getModel());
			else
				EResourceSystemPanelV2.this.unselect(getModel());
		}
		public Boolean isSelected() {
			return EResourceSystemPanelV2.this.isSelected(getModel());
		}
...
//					}
			...
			if (((ResourceNode)model.getObject()).getResource() instanceof ResourceFolder) {
				return new FolderViewPanel<Content>("resource-view", model) {
					public ViewMode getViewMode() {
						return EResourceSystemPanelV2.this.getViewMode();
					}
					public void onClick(AjaxRequestTarget target) {
				...
				Panel view = new ResourceViewPanel<Content>("resource-view", model, getContentModel()) {
					...
					public boolean isShared() {
						return EResourceSystemPanelV2.this.isShared();
					}
				};
				view.add(new DraggableBehavior() {
					protected Component getContainment() {
						return EResourceSystemPanelV2.this.get("container:control:browser");
					}
				});
		...
		protected WebMarkupContainer getMenu(IModel<Resource> model) {
				...
				new UploadMenuItemPanelV5<Resource>(id) {
					...
					protected String getUploadUrl() {
						...
						return "/versionupload?id="+ content.getId() +
							"&class="+classname+
							"&resource="+resourceid+
							"&path="+EResourceSystemPanelV2.this.getPath();
					}	
					@Override
					protected String getRefreshFunction() {
						String id = getEditor(EResourceSystemPanelV2.this).getMarkupId();
						//UploadBehavior behavior = getBehaviors(UploadBehavior.class).get(0);
						return "refreshfiles"+id+"('"+EResourceSystemPanelV2.this.getMarkupId()+"');";
					}
					@Override
					protected Component getResourcesView() {
						return EResourceSystemPanelV2.this.get("container:control:browser:list-view:resources-view");
					}
					@Override
					public boolean isVisible() {
			...
			menu.addItem(id ->
				new AjaxMenuItemPanelV5<Resource>(id) {
					@Override
					public void onClick(AjaxRequestTarget target) {
						IModel<String> ms = EResourceSystemPanelV2.this.getLabel("dialog.delete.message", getModelObject().getTitle());
	 ...
	 * 
	 */
	public EResourceSystemPanelV2(String id, KbeeEResourceSystemV2 field, IModel<EFormData> data) {
		super(id, field, data);
		setOutputMarkupId(true);
		try {
	...
	public void onInitialize() {
		...
		add(new UploadBehavior() {
			@Override
			public boolean isEnabled() {
				return EResourceSystemPanelV2.this.isEnabled();
			}
			@Override
			protected String getUrl() {
				return "/formupload?path="+EResourceSystemPanelV2.this.getPath();
			}
			@Override
			...
			protected String getBrowseButton() {
				return EResourceSystemPanelV2.this.get("container:control:pickfiles").getMarkupId();
			}
		});
		...
		add(new WicketEventListener<EAjaxRefreshEvent>() {
			...
			public boolean handle(EAjaxRefreshEvent event) {
				return EResourceSystemPanelV2.this.getMarkupId().equals(event.getComponentId());
			}
		});
	...
	private Panel getMenu() {
		...
		menu.addItem(id ->
			new AjaxMenuItemPanelV5<Void>(id) {
				@Override
				public void onClick(AjaxRequestTarget target) {
				IModel<String> ms = EResourceSystemPanelV2.this.getLabel("dialog.deleteall.message");
				getConfirmationDialog().open(target, ms, Dialog.Delete, new Dialog.Handler() {
					@Override
					public void onClick(AjaxRequestTarget target, Button button) {
						if (button.key().equals(Dialog.Delete.key())) {
