...
public class EListTreePanel<T> extends EFieldPanel<EListField<T>> {
	...
	public class ControlFragment extends Fragment {
		...
		public ControlFragment(String id) {
			super(id, "control-fragment", EListTreePanel.this);
			...
			add(new AutoCompleteFieldV5<T>("field", new PropertyModel<T>(EListTreePanel.this, "value")) {
				@Override
				...
				public void onUpdate(AjaxRequestTarget target) {
					...
					EListTreePanel.this.validate(target, getValue());
					if (!hasFeedback()) {
						addValue(getValue());
						setValue(null);
						setSuggestion(null);
				...
				public boolean hasFeedback() {
					return !EListTreePanel.this.getMessages().isEmpty();
				}
				@Override
				public String getMessage() {
					return hasFeedback() ? EListTreePanel.this.getMessages().get(0).toString() : null;
				}
				@Override
				...
				protected String getValue(Suggestion suggestion) {
					...
					return object != null 
						? EListTreePanel.this.getValue(object)
						: suggestion.getText();
				}
				@Override
				@SuppressWarnings("unchecked")
				protected String getInfo(Suggestion suggestion) {
					...
					String info = suggestion.getObject() != null 
						? EListTreePanel.this.getInfo(object)
						: null;
					return info;
				}
				@Override
			...
			treeSelector = new ETreeNodeSelector("selector", 
					new ObjectModel<DataSet>(dataSet),
					((EClassifierModel<?>)getField().getModel()).getAccessStrategy()) {
				@Override
				@SuppressWarnings("unchecked")
				protected void onSelect(AjaxRequestTarget target, DataSetMember member) {
					...
					setTree(false);
					fireScanAll(new EAjaxFormEvent(target, getField(), getData()));
					EListTreePanel.this.refresh(target);
				}
				@Override
	 ...
	 * @param data
	 */
	public EListTreePanel(String id, EListField<T> field, IModel<EFormData> data) {
		super(id, field, data);
	}
	...
	public void onInitialize() {
		super.onInitialize();
		if (getContainer().get("control")!=null) {
			return;
		}
		setValues();
	...
	protected Panel getMenu() {
		...
		new AjaxMenuItemPanelV5<Void>(id) {
			...
			@Override
			public String getLabel() {	
				return getLabelString("menu.tree");
			}
			@Override
			public boolean isEnabled() {
	...
	protected void validate(AjaxRequestTarget target, T value) {
		...
		getField().validate(new KbeeEValidatable(getData(), getField()) {
			...
			public void onError(String key, String... parameter) {
				String message = getLabelString(key, parameter);
				addMessage(message);
				((Panel)getEditor()).error(new FieldMessage(EListTreePanel.this, getData().getForm(), getField(), message, FeedbackMessage.ERROR));
			}
		});
	}
	...
	protected void setTree(boolean open) {
		if (getDisposition()==null || getDisposition()==Disposition.HORIZONTAL) {
			getContainer().get("horizontal-layout:control:selector").setVisible(open);
		}
		else {
			getContainer().get("control:selector").setVisible(open);
		}
	}
