...
//public class AdvancedSearchForm extends AdvancedSearchSelectorEditor<Void> {
public class AdvancedSearchForm extends KBPanel {
	private static final long serialVersionUID = 1L;

	static private Logger logger = Logger.getLogger(AdvancedSearchForm.class.getName());
	...
	public AdvancedSearchForm(String id) {
		super(id);
...
//	static final String css_st[] = {"col-lg-12", "col-lg-12", "col-lg-6", "col-lg-4", "col-lg-3", "col-lg-2", "col-lg-2", "col-lg-6"};
	...
	public void onInitialize() {
		...
		TextField<String> t1 = new TextField<String>("title", new PropertyModel<String>(this, "title"));	
		t1.setPlaceholderLabel(false);
		t1.listenEnter(listenEnter());
		form.add(t1);
...
//					item.add( new AttributeModifier("class", css_1 + " col-md-6 col-xs-12 ")); 
				...
				if (element instanceof Attribute) {
					if ( ((Attribute) element).isDate()) {
 						DateTimeField df = new DateTimeField("field",	
 							ZoneId.systemDefault(),// sessionuser().getZoneId(), 
 							(IModel<OffsetDateTime>) item.getModelObject(), false) {
						...
						((TextField<?>)item.get("field")).listenEnter(listenEnter());
					}
				}	
				...
				if (element instanceof Classifier) {
					item.add(new AutoCompleteFieldV5<DataSetMember>("field", (IModel<DataSetMember>)item.getModelObject(), false) {
						...
						public String getHistoryKey() {
							return AdvancedSearchForm.this.getKey()+"-advanced-search-"+item.getModelObject().getElement().getId();
						}
						@Override 
						public IModel<String> getLabel() {
							return new Model<String>(item.getModelObject().getElement().getDisplayName());
		...
		AjaxButton sl =new AjaxButton("submit") {
			@Override
			protected void onSubmit(AjaxRequestTarget target) {
				AdvancedSearchForm.this.onSubmit(target);
			}
			@Override
				...
				IAjaxCallListener listener = new IAjaxCallListener() {
					...
					public CharSequence getBeforeHandler(Component component) {
						return "document.getElementById('"
								...
								+"').innerHTML = '<span class=\"far fa-sync fa-spin fa-fw spinning\"></span> "
								+AdvancedSearchForm.this.getLabel("working").getObject() +"'";
					}
					@Override
...
//			sl.add( new AttributeModifier("style", bu));
		...
		form.add(sl);
		if (listenEnter()) {
			form.setDefaultButton(sl);
		}
		
		form.add(new AjaxLink<Void>("simple") {
			@Override
			public void onClick(AjaxRequestTarget target) {
				AdvancedSearchForm.this.onClose(target);
	...
	private String getDefaultDomainPreference(String key, String defaultValue) {
		...
		if (domain==null || key==null)
			return defaultValue;
		return domain.getService(DomainPreferencesService.class).getValue("advancedsearch", key, defaultValue);
	}
