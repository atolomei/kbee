...
;(function (undefined) {
	...
	Wicket.AutoComplete=function(elementId, callbackUrl, cfg, indicatorId){
		...
		var myshow = false; // on FF, clicking an option in the pop-up would make field loose focus; focus() call only has effect in FF after popup is hidden, so the re-focusing must not show popup again in this case
		...
		this.show = function () {
			cfg.showListOnEmptyInput=true;
			cfg.showListOnFocusGain = true;
			minInputLength = 0;
		...
		function initialize(){
			...
			Wicket.Event.add(obj, 'blur', function (jqEvent) {
				cfg.showListOnEmptyInput=false;
				cfg.showListOnFocusGain = false;
				hidetimeout = window.setTimeout(hideAutoComplete, 500);
			});
		...
		function actualUpdateChoicesShowAll() {
			...
			Wicket.Ajax.ajax(attrs);
				...
				function actualUpdateChoicesShowAll2() {
			showIndicator();
