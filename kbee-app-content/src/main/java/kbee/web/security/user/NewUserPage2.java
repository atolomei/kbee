...
public class NewUserPage2 extends ConsoleObjectPage<NewUserData> {
	...
	public NewUserPage2(IModel<User> b) {
		setBaseUser(b);
	...
	public NewUserPage2(IModel<NewUserData> model, IModel<User> b) {
		super(model);
		setBaseUser(b);
		addComponents(model);
	}
	...
	private void addComponents(IModel<NewUserData> model) {
		...
		if (hasPermissions()) {
			...
			setPageTitle(new StringResourceModel("new-user", NewUserPage2.this, null));
			setPageDescription(getPageTitle());

			PageContentHeaderPanel<NewUserData> panel=new PageContentHeaderPanel<NewUserData>(null);
			panel.setTitle( new StringResourceModel("new", NewUserPage2.this, null));
			...
			bc.addElement(new BCElement(new StringResourceModel("new", NewUserPage2.this, null)));
			panel.setBreadcrumbPanel(bc);
			
			add(new NewUserFormEditor("editor", model) {
				@Override
				public void onEdit(IModel<Person> model) {
