...
public class RolePermissionsEditor extends DomainObjectEditor<Role> {
	...
	public RolePermissionsEditor(String id, IModel<Role> model, boolean isnew, boolean isWorkflow) {
		super(id, model);
	...
	public void onInitialize() {
		...
		WebMarkupContainer rwd=new WebMarkupContainer("read-write-delete");
		rwd.setVisible(!isWorkflow);
		form.add(rwd);
		
		
		AlertPanel<Void> pa=new AlertPanel<Void>("description",AlertPanel.INFO,  null, 
				null, 
				new StringResourceModel(isWorkflow?"workflow-permissions":"content-permissions", this, null));
		...
		form.add((new PermissionsPanel("permissions-editor", this.isWorkflow)));
	...
	private IModel<String> getTitle() {
		return this.isWorkflow ? 
					new StringResourceModel("workflow-title", this,null) : 
					new StringResourceModel("content-title", this,null);
	}
	...
	public void update(AjaxRequestTarget target) {
		try {
			if (!getUpdatedParts().isEmpty()) {
				...
				super.reset();
				target.add(RolePermissionsEditor.this.getPage());
			}
		}
		catch (Exception e) {
			logger.error(e);
