...
public class TaskKnowledgeBasePanelV5<T extends Content>  extends ModelPanel<WorkflowContext> {
	...
	public TaskKnowledgeBasePanelV5(IModel<WorkflowContext> model) {
		this ("workflow-info", model);
	}
	
	public TaskKnowledgeBasePanelV5(String id, IModel<WorkflowContext> model) {
		super(id, model);
		...
		if (get("content")==null) {
			...
			AjaxLink<Void> vql = new AjaxLink<Void>("view-iql-query") {
				...
				public void onClick(AjaxRequestTarget target) {
					TaskKnowledgeBasePanelV5.this.get("iql-query").setVisible(
							! TaskKnowledgeBasePanelV5.this.get("iql-query").isVisible());
					TaskKnowledgeBasePanelV5.this.get("instanced-iql-query").setVisible(
							! TaskKnowledgeBasePanelV5.this.get("instanced-iql-query").isVisible());
					target.add(TaskKnowledgeBasePanelV5.this);
				}
			};
		...
		add(new ListView<IModel<Content>> ("content", getContent()) {
			...
			public void populateItem(ListItem<IModel<Content>> item) {
				...
				Link<Content> ln = new Link<Content>("title-link", item.getModel().getObject()) {
					...
					@Override
					public void onClick() {
						Page page = TaskKnowledgeBasePanelV5.this.getPage(getModel()); 
						if (page!=null) {
	...
	protected Panel getNavigationPanel(IModel<Content> model, long index) {
		Panel panel = new ContentNavigationBar<Content>("navigation", model,  new Searcher(getQuery()), index) {
			...
			public void onNavigate(Content content) {
				...
				setModel(model);
				Page page = TaskKnowledgeBasePanelV5.this.getPage(model);
