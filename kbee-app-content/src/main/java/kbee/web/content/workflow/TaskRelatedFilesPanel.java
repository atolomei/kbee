...
@Deprecated
public class TaskRelatedFilesPanel<T extends Content> extends ModelPanel<WorkflowContext> {
			
	private static final long serialVersionUID = 1L;

	private List<IModel<Content>> contentlist;

	private static kbee.util.logging.Logger logger = kbee.util.logging.Logger.getLogger(TaskRelatedFilesPanel.class.getName());
	
	private Map<Integer, Boolean> item_expanded = new HashMap<Integer, Boolean>();
	private ContextCriteriaQuery query;

	public TaskRelatedFilesPanel(String id, IModel<WorkflowContext> model) {
		super(id, model);
		...
		if (get("content")==null) {
			...
			AjaxLink<Void> vql = new AjaxLink<Void>("view-iql-query") {
				...
				public void onClick(AjaxRequestTarget target) {
					TaskRelatedFilesPanel.this.get("iql-query").setVisible(
							! TaskRelatedFilesPanel.this.get("iql-query").isVisible());


					TaskRelatedFilesPanel.this.get("instanced-iql-query").setVisible(
							! TaskRelatedFilesPanel.this.get("instanced-iql-query").isVisible());
					target.add(TaskRelatedFilesPanel.this);
			...
			Label vqi = new Label("instanced-iql-query", new Model<String>() { 
				public String getObject() {
				   if (TaskRelatedFilesPanel.this.query!=null) {
					   return "<b>query parameters</b> -> " + TaskRelatedFilesPanel.this.query.getQuery().getParameters().toString();
				   }
					return "null";	
				}
			});
			...
			public void populateItem(ListItem<IModel<Content>> item) {
				...
				WebMarkupContainer icon = new WebMarkupContainer("icon");
						icon.add(new AttributeModifier("class", new Model<String>() {
							@Override
							public String getObject() {
								return isExpanded(item.getIndex())?"far fa-angle-up fw":"far fa-angle-down fw";
							}
				}));
				link.add(icon);
				item.add(link);
				...
				Link<Content> ln = new Link<Content>("title-link", item.getModel().getObject()) {
					...
					@Override
					public void onClick() {
						Page page = TaskRelatedFilesPanel.this.getPage(getModel()); 
						if (page!=null) {
...
//				else
	...
	protected Panel getNavigationPanel(IModel<Content> model, long index) {
		Panel panel = new ContentNavigationBar<Content>("navigation", model,  new Searcher(getQuery()), index) {
			...
			public void onNavigate(Content content) {
				...
				setModel(model);
				Page page = TaskRelatedFilesPanel.this.getPage(model);
	...
	protected Page getPage(IModel<Content> model) {
		...
		Page page = (Page)ServiceLocator.getService(BeansService.class).getBean(ProxyUtil.getClassName(model.getObject()).toLowerCase() +  "-page" , model, false);
		return page;
	}
