...
public class ArchiveTools extends ToolbarItem {
		
	private static final long serialVersionUID = 1L;
	
	private BaseBrowser<Content> browser;
	
	public ArchiveTools(BaseBrowser<Content> browser, Align align) {
		super(browser, align);
			...
			public AbstractMenuItemPanelV5<Void> getItem(String id) {
				return new AjaxMenuItemPanelV5<Void>(id) {
					...
					public String getLabel() {
						return ArchiveTools.this.getLabel("tools.checkout").getObject();
					}
					@Override
		...
		menu.addItem(new MenuItemFactory<Void>() {
			...
			public AbstractMenuItemPanelV5<Void> getItem(String id) {
				return new AjaxMenuItemPanelV5<Void>(id) {
					...
					@Override
					public String getLabel() {
						return ArchiveTools.this.getLabel("tools.movetocontentbase").getObject();
					}
					@Override
					public boolean isEnabled() {
						return !getSelection().isEmpty();
		...
		menu.addItem(new MenuItemFactory<Void>() {
			...
			public AbstractMenuItemPanelV5<Void> getItem(String id) {
				return new AjaxMenuItemPanelV5<Void>(id) {
					...
					@Override
					public String getLabel() {
						return ArchiveTools.this.getLabel("tools.delete").getObject();
					}
					@Override
					public boolean isEnabled() {
						return !getSelection().isEmpty();
		...
		add(new WicketEventListener<SelectionEvent>() {
			...
			public void onEvent(SelectionEvent event) {
				event.getRequestTarget().add(ArchiveTools.this);
			}
		});
	}
