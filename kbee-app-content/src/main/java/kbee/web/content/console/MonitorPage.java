...
import com.novamens.kbee.portal.service.MonitorSearchSuggestionService;
...
public class MonitorPage extends ContentConsolePage<Content> {
			
	private static final long serialVersionUID = 1L;
	private static kbee.util.logging.Logger logger = kbee.util.logging.Logger.getLogger(MonitorPage.class.getName());
	...
	public MonitorPage(Query query) {
		super(query);
	}
	...
	public void onInitialize() {
		...
		panel.setTitle(new StringResourceModel("monitor", this, null));
		panel.setBreadcrumbPanel(getContentHeaderPanelBreadcrumbPanel());
		panel.setMenuPanel(getContentHeaderPanelMenuPanel());

		
		setSearchPlaceHolder(new StringResourceModel("search-in-monitor", this, null).getObject());
	...
	protected Panel getContentHeaderPanelBreadcrumbPanel() {
		try {
			...
			bc.addElement(new TasksDropDownMenuBC());
 			bc.addElement(new BCElement("monitor"));
			return bc;
		} catch (Exception e) {
	...
	protected List<Suggestion> getSuggestions(String pattern) {
		return getDomain().getService(MonitorSearchSuggestionService.class).getSuggestions(pattern); 
	}
	

	/** 
	 * These urls must be included in the  {@code applicationContext.xml}
	 * and mapped by {@code BaseWebApplication}
	 */
	@Override
	...
	public String hasPermissionsReason() {
		...
		str.append("If you are none of them: you need <b>Monitor</b> enabled in your <b><a class=\"btn-link\" href=\"/myaccount\" target=\"_blank\">Rights</a></b>.");
		return str.toString();
	}
}
