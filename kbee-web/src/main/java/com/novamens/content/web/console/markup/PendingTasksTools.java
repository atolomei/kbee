...
public class PendingTasksTools extends ToolbarItem {
	...
	public PendingTasksTools(BaseBrowser<Content> browser, Align align) {
		super(browser, align);
			...
			public AbstractMenuItemPanelV5<Void> getItem(String id) {
				return new AjaxMenuItemPanelV5<Void>(id) {
					...
					public String getLabel() {
						return PendingTasksTools.this.getLabel("tools.take").getObject();
					}
					@Override
		...
		add(new WicketEventListener<SelectionEvent>() {
			public void onEvent(SelectionEvent event) {
				event.getRequestTarget().add(PendingTasksTools.this);
			}
		});
	}
