...
public class PermissionsTab<T> extends com.novamens.wicket.markup.html.editor.ObjectEditorPanel<T>  {
		...
		public PermissionFragment(String id, final AclRow row, final Permission permission) {
			...
			AjaxLink<?> permissionLink = new AjaxLink<Void>("permission-link") {
				...
				public boolean isEnabled() {
					return PermissionsTab.this.getEditor().isEditionEnabled();
				}
			};
	 ...
	 * @param readOnly
	 */
	public PermissionsTab(String id, Editor<T> editor, AclRow row, List<Permission> permissions) {
		super(id);
		//setReadOnly(readOnly);
		setEditor(editor);
		setOutputMarkupId(true);
		this.row = row;
		DataTable<AclRow, String> table = new AjaxFallbackDefaultDataTable<AclRow, String>("rows", getColumns(permissions), new AclRowsProvider(), 20);
		add(table);
	}
