...
public class TestCommandsPanel extends KBPanel {
			
    private static final long serialVersionUID = 1L;

    private static kbee.util.logging.Logger logger = kbee.util.logging.Logger.getLogger(TestCommandsPanel.class.getName());
    ...
    public TestCommandsPanel(String id) {
        super(id);
       ...
       form.add(parameterPanel);
        ...
        form.add(new ChoiceField<Bean>("command", new PropertyModel<Bean>(this, "command"), new PropertyModel<List<Bean>>(this, "commands")) {
            ...
            public void onUpdate(AjaxRequestTarget target) {
                setCommand(getValue());
                TestCommandsPanel.this.statusPanelBinded = false;
                TestCommandsPanel.this.getStartButton().setEnabled(true);
                String d=((Bean) getValue()).getDescription();
                setDescription(d!=null?d:((Bean) getValue()).getDisplayName());
              
             commandParams=      TestCommandsPanel.this.callCommandParameters();
               ...
               target.add(TestCommandsPanel.this);
            }
    ...
    private Component getStartButton() {
        return TestCommandsPanel.this.get("form").get("start-button");
    }
