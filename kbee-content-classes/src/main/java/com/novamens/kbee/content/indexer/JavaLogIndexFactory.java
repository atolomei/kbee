...
public class JavaLogIndexFactory implements JavaIndexFactory, EventListener, BeanNameAware {
	...
	private Index getIndexBean(Domain domain) {
 		if (!ServiceLocator.getService(BeansService.class).containsBean(domain.getName()+"-audit-index"))
			return null;
		else
	...
	private List<DocumentSchema> getSchemas(Index index, Domain domain) {
		...
		schemas.add(createLogEventSchema(domain));
		return schemas;
	}
	
	private DocumentSchema createLogEventSchema(Domain domain) {
		return getDefaultLogEventSchema();
	}
	...
	private DocumentSchema getDefaultLogEventSchema() {
		return (DocumentSchema)ServiceLocator.getService(BeansService.class).getBean("default-logevent-schema");
	}
}
