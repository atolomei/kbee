 ...
 *
 */
public class ParentsDisplayNameExtractor implements Extractor, EventListener {
			...
			for (Classification classification : content.getClassification()) {
				...
				if (classifier!=null && classifier.equals(getClassifier())) {
					...
					if (classifier.getDataSet()!=null &&  classifier.getDataSet().getDataSetType().equals(DataSetType.DATE)) {
						...
						if (member!=null) {
							members.addAll(getParents(member));
						}
						break;
					}
				}
					...
					if ("strvalue".equals(propertyName)) {
						if (updateevent.getCurrentState()[i]!=null && 
							...
							!updateevent.getCurrentState()[i].equals(updateevent.getPreviousState()[i])) {
								...
								query.getParameters().put(getIdFieldName(), "["+
	...
	protected List<String> getParents(DataSetMember member) {
		return getParents(member, new HashSet<>());
	}
	
	protected List<String> getParents(DataSetMember member, Set<Serializable> parents) {
		List<String> names = new ArrayList<>();
		parents.add(member.getId());
		for (DataSetMember parent : member.getParents()) {
			names.add(parent.getDisplayName());
			if (!parents.contains(parent.getId())) {
				names.addAll(getParents(parent, parents));
			}
		}
		return names;
	}
