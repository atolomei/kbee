...
public class FileIndexerServiceFactory extends AbstractServiceFactory<IndexerService> {
	
	private JavaIndexFactory factory;
	
	public boolean isFactory(Class<? extends Service> serviceClass) {
		return serviceClass.equals(FileIndexerService.class);
	}
	
	@SuppressWarnings("unchecked")
	public <S extends ObjectService> S getService(Object object) {
		Assert.isInstanceOf(Domain.class, object);
		FileIndexerService service = new FileIndexerService(getIndex((Domain)object));
		return (S)service;
	}
