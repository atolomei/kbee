...
public class ContentGroupsExtractor implements Extractor {
	...
	public Object extract(Object object) throws IndexerException  {
		...
		Long userid = content.getWorkspace();
		
		User user;
		
		if (userid!=null) {
			user = ServiceLocator.getService(SecurityService.class).findUserById(userid);
		}
		else {
			user = content.getLastModifiedUser();
		}	
