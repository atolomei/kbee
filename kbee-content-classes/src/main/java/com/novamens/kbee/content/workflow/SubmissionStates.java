...
@Deprecated
public class SubmissionStates extends KbeeStatesMachine {
	
	public final static String Submission_Task = "Submission";
	public final static String FileReview_Task = "File Review";
	public final static String Resubmission_Task = "Resubmission";
	public final static String Cancelation_Task = "Cancelation";
	public final static String Acknowledge_Cancel_Task = "Acknowledge Cancel";
	public final static String Collaboration_Task = "Collaboration";
	public final static String Acknowledge_Task = "Acknowledge";
	public final static String FinalReview_Task = "Final Review";
	public final static String FinalSubmission_Task = "Final Submission";
	...
	public final static String Acknowledge_Response = "acknowledge_response"; 
	public final static String Acknowledge_Timeout_Response = "acknowledge_timeout_response";
	public final static String Acknowledge_Cancel_Response = "acknowledge_cancel_response";
	...
	public Activity handle(String event, WorkflowContext context) {
		...
		case StatesMachine.Initialization_Event:
			task = procedure.getTask(Submission_Task);
		...
		case Resubmission_Response:
			...
			task.getTrigger().pull(context);
			break;
		case FinalSubmission_Request:
			task = procedure.getTask(FinalSubmission_Task);
			task.getTrigger().pull(context);
			break;
		case FinalSubmission_Response:
			task = procedure.getTask(FinalReview_Task);
			task.getTrigger().pull(context);
			break;
		case Submitter_Cancel_Request:
			task = procedure.getTask(Cancelation_Task);
			task.getTrigger().pull(context);
			break;
		case Review_Cancel_Request:
			task = procedure.getTask(Acknowledge_Cancel_Task);
			task.getTrigger().pull(context);
			break;
		...
		case Approve_Request:
			task = procedure.getTask(Acknowledge_Task);
			task.getTrigger().pull(context);
			break;
		case Acknowledge_Response:
		case Acknowledge_Timeout_Response:
			task = procedure.getTask(FinalReview_Task);
			task.getTrigger().pull(context);
			break;
		case ApproveAndPublish_Request:
		case Acknowledge_Cancel_Response:
...
//		Map<Role, List<Principal>> roles = new HashMap<Role, List<Principal>>();
//		
//		KbeeContext kbeecontext = (KbeeContext)context;
//		
//		Content content = (Content)getContentDao().reload(kbeecontext.getContent());
//		KbeeProcedure procedure = (KbeeProcedure)kbeecontext.getProcedure();
//		
//		List<Principal> principals1 = new ArrayList<Principal>();
//		
//		KbeePermission permission = null;
//		
//		Task task = procedure.getTask(FileReview_Task);
//		
//		if (task.getTrigger() instanceof UserAutomaticTrigger) {
//			UserAutomaticTrigger trigger = (UserAutomaticTrigger)task.getTrigger();
//			if (trigger.getUserAssignationStrategy() instanceof KbeeRoundRobin) {
//				KbeeRoundRobin roundrobin = (KbeeRoundRobin)trigger.getUserAssignationStrategy();
//				permission = (KbeePermission)roundrobin.getPermission();
//			}
//		}
//		
//		List<Principal> principals2 = new ArrayList<Principal>();
//		
//		if (permission!=null) {
//			principals2.addAll(getEnabledPrincipals(content, permission));
//		}
//		
//		
//		roles.put(Analyst_Role, principals2);
//		roles.put(Submitter_Role, principals1);
//
//		return roles;
//	}
//
