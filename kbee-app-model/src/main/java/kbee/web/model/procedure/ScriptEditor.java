...
public class ScriptEditor<T> extends ObjectEditorPanel<T> {
	...
	class ScriptValidator implements IValidator<String> {
		...
		public void validate(final IValidatable<String> validatable) {
			...
			String message = executor.validate(script, getTemplate());
			if (message!=null) {
	...
	public void onInitialize() {
		...
		IModel<String> model = new PropertyModel<String>(getModel2(), property);

		add(new TextAreaField<String>("script", model, false, Width.W12, new ScriptValidator(), 4, 40) {
			@Override
			...
			public String getProperty() {
				return ScriptEditor.this.property;
			}
			@Override
	...
	private IModel<String> getScriptHelp() {
		return new Model<String>(JsEvaluator.GetHelpText(getTemplate()));
	}
