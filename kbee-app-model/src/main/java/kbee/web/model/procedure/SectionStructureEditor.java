...
import kbee.web.model.contentclass.AbstractStructureEditor;

@Deprecated
@SuppressWarnings("serial")
public class SectionStructureEditor<T> extends AbstractStructureEditor<T> {
	...
	public SectionStructureEditor(IModel<ModelSection> model, IModel<ContentTemplate> templateModel) {
		super(model);
		this.templateModel = templateModel;
	}
	
	public SectionStructureEditor(String id, IModel<ModelSection> model, IModel<ContentTemplate> templateModel) {
		super(id, model);
		this.templateModel = templateModel;
	}
	...
	protected List<Property<?>> getProperties() {
			...
			public String getDisplayValue(ModelElementTemplate template) {
				...
				if (template instanceof ClassifierTemplate) {
					 ...
					 if(c!=null) {
						 ...
						 if (d!=null) {
							 if (d.isAggregation()) {
								 return template.getDisplayName()+" <span class=\"ago\">("+ new StringResourceModel("built-in", SectionStructureEditor.this, null).getObject()+")</span>";
							 }
						 }
					 }
			...
			public String getValue(ModelElementTemplate value) {
				if (value instanceof ClassifierTemplate) {
					...
					if (((ClassifierTemplate)value).isReverse()) {
						label += SectionStructureEditor.this.getLabel("reverse").getObject(); // "Reverse ";
					}
					if (value.getElement()!=null &&
						 ((Classifier)value.getElement()).getDataSet() instanceof EntitySet) {
						return label + SectionStructureEditor.this.getLabel("classifier").getObject(); // Classifier ;
					}
					else {
						return label + SectionStructureEditor.this.getLabel("classifier").getObject();
					}
				}
				else if (value instanceof AttributeTemplate) {
					String label = SectionStructureEditor.this.getLabel("attribute").getObject(); // "Attribute";
					if (value.getElement()!=null) {
						label = label + " <span class=\"ago\">( " + ((Attribute)value.getElement()).getType().getLabel()+" )</span>" ;
					}
					return label;
				}
				else if (value instanceof SubsectionTemplate) {
					return "<b>"+ SectionStructureEditor.this.getLabel("subsection").getObject()+"</b>";
				}
				...
				if (value instanceof ModelElementTemplate) {
					if (((ModelElementTemplate)value).getElement() instanceof Classifier) {
						...
						if (((ModelElementTemplate)value).isReverse()) {
							label += SectionStructureEditor.this.getLabel("reverse").getObject()+" ";
						}
						if (value.getElement()!=null &&
							 ((Classifier)value.getElement()).getDataSet() instanceof EntitySet) {
							return label + SectionStructureEditor.this.getLabel("classifier").getObject(); //(Entity) ";
						}
						else {
							return label + SectionStructureEditor.this.getLabel("classifier").getObject();
						}
					...
					if (((ModelElementTemplate)value).getElement() instanceof Attribute) {
						String label = SectionStructureEditor.this.getLabel("attribute").getObject();
						label = label + " <span class=\"ago\">( " + ((Attribute)value.getElement()).getType().getLabel()+" )</span>" ;
						return label;
					}
				}
		...
		properties.add(new Property<AccessStrategy>() {
			...
			@Override
			public boolean isVisible(IModel<ModelElementTemplate> model) {
				return model.getObject().getElement()!=null && model.getObject().getElement() instanceof Classifier;
			}
			@Override
			public String getStringWidth() {
				return "10%";
		...
		/**
		 * User enters a value or calculated by  JS Script
		 * 
		 * 
		 * 
		 * 	duedate = today.plusDays(10)
			context.setDueDate(duedate)
			content.setValue("fechavencimiento", duedate)
		...
		properties.add(new TextAreaProperty() {
			...
			public void onHelp(AjaxRequestTarget target) {
				SectionStructureEditor.this.onHelp(target); 
			}
		});
