...
public class TaskForkJoinMainPanel extends ObjectEditor<Task>  implements PageMainTabs {
	private static final long serialVersionUID = 1L;
			
	static private Logger logger = Logger.getLogger(TaskForkJoinMainPanel.class.getName());

//	private IModel<Procedure> proc_model;
	
	public TaskForkJoinMainPanel(IModel<Task> model) {
		super("editor", model);
	...
	protected VerticalLayout<ITab> getTabs() {
		...
		tabs.add(new AbstractTabKB(getLabel("editor.task"), "task") {
			...
			public Panel getPanel(String panelId) {
				return new TaskForkJoinEditor(panelId, getModel());
			}	
		});
		
		tabs.add(new AbstractTab(getLabel("editor.threads")) {
			@Override
			public Panel getPanel(String panelId) {
				return new TaskThreadsEditor(panelId, getModel());
			}
		});
...
//			@Override
//			public Panel getPanel(String panelId) {
//				return new TaskFormsEditor(panelId, proc_model, getModel());
//			}
//		});
//		
