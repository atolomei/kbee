...
public class LauncherGroupsPage extends ConsolePage<LauncherGroup> {
	...
	public LauncherGroupsPage() {
		setPageTitle(getLabel("launchergroups"));
		KbeeUser user = (KbeeUser) getSessionUser();
		if (user!=null) 
			user.getService(PreferencesService.class).setValue( "settings", "informationmodel", LauncherGroupsPage.class.getName());
	}
	
	public LauncherGroupsPage(Query query) {
		super(query);
		setPageTitle(getLabel("launchergroups"));
		KbeeUser user = (KbeeUser) getSessionUser();
		if (user!=null) 
	...
	public void onInitialize() {
		...
		panel.setBreadcrumbPanel(new InformationModelBCPanel("bc.launchergroups"));
		setPageTitle(getLabel("bc.launchergroups"));
		panel.setTitle(getLabel("bc.launchergroups"));
		setSearchPlaceHolder(getLabel("search-in").getObject().replace("{0}", getLabel("bc.launchergroups").getObject()));
		setSearchPanel(true);
	...
	@Override
	public Console<LauncherGroup> newConsole(Query query) {
		return new LauncherGroupsConsole(query) {
			@Override
			public Page getConsolePage(Query query, long index) {
				return LauncherGroupsPage.this.getConsolePage(query, index);
			}
		};
	}
	...
	@Override
	public Page getConsolePage(Query query, long index) {
		return new LauncherGroupsPage(query);
	}
