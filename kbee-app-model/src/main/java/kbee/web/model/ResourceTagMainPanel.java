...
public class ResourceTagMainPanel extends DomainObjectEditor<ResourceTag> implements PageMainTabs  {
	private static final long serialVersionUID = 1L;
	
	private static Logger logger = Logger.getLogger(ResourceTagMainPanel.class.getName());

	private String initial_tab;
	
	public ResourceTagMainPanel(IModel<ResourceTag> model, final boolean editon, final boolean is_new) {
		super("editor", model);
		...
		tabs.add(new AbstractTabKB(getLabel("editor.info"), "info") {
			...
			public Panel getPanel(String panelId) {
				return new ResourceTagEditor(panelId, getModel(), is_new) {
					@Override
					public void onUpdate(AjaxRequestTarget target) {
						//((ClassifierHeaderPanel)ResourceTagMainPanel.this.get("dataset-panel")).onUpdate(target);
					}
					protected void onClose(AjaxRequestTarget target) {
						ResourceTagMainPanel.this.onClose(target);							
					}
				};
			}
		});
		...
		tabs.add(new AbstractTabKB(getLabel("editor.state"), "status") {
			...
			public Panel getPanel(String panelId) {
				...
				return new ObjectStateEditor<ResourceTag>(panelId, getModel(), real_only);
			}
		});
		...
		tabs.add(new AbstractTabKB(getLabel("editor.audit"), "audit") {
			@Override
			public Panel getPanel(String panelId) {
				return new AuditTrailObjectPanel<ResourceTag>(panelId, new ObjectModel<ResourceTag>(getModel().getObject()));
			}
		});
