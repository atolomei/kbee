...
public class ResourceTagsQuery extends HibernateQuery {
	private static final long serialVersionUID = 1L;
			
	static private kbee.util.logging.Logger logger = kbee.util.logging.Logger.getLogger(ResourceTagsQuery.class.getName());
	boolean deleted_visible = true;
	
	public ResourceTagsQuery() {
	}			
	
	public ResourceTagsQuery(boolean deleted_visible) {
	...
	public String getStatement() {
		...
		statement.append("from KbeeResourceTag T where T.domain.id=" + String.valueOf(domain.getId()) +"  " + h_text.toString());
		
		String sizeQuery = "select count (*) FROM KbeeResourceTag T WHERE T.domain.id=" + domain.getId().toString() +" "+ h_text.toString();
