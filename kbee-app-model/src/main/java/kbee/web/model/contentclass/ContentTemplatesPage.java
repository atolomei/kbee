...
import kbee.web.model.InformationModelBCPanel;
import kbee.web.page.ApplicationMenuSection;
import kbee.web.page.PageContentHeaderPanel;
import kbee.web.workflow.task.PageTaskToolbar;

@SuppressWarnings("serial")
public class ContentTemplatesPage extends ConsolePage<ContentTemplate> {
	...
	public ContentTemplatesPage(Query query) {
		super(query);
	}
	...
	public void onInitialize() {
		...
		setPageTitle(getLabel("contentclasses"));
		
		KbeeUser user = (KbeeUser) getSessionUser();
		if (user!=null) 
			user.getService(PreferencesService.class).setValue( "settings", "informationmodel", ContentTemplatesPage.class.getName());
		
		PageContentHeaderPanel<?> panel=new PageContentHeaderPanel<Void>();
		panel.setBreadcrumbPanel(new InformationModelBCPanel("bc.contentclasses"));
		setPageTitle(new StringResourceModel("bc.contentclasses", this, null));
		panel.setTitle(new StringResourceModel("bc.contentclasses", this, null));
	...
	@Override
	public Console<ContentTemplate> newConsole(Query query) {
		return new ContentTemplatesConsole(query) {
			@Override
			public Page getConsolePage(Query query, long index) {
				return ContentTemplatesPage.this.getConsolePage(query, index);
			}
		};
	}
	...
	public boolean hasPermissions() {
		return is_domain_admin || is_root || is_support || is_model || is_model_read; 
	}
	
	@Override
	public Page getConsolePage(Query query, long index) {
		return new ContentTemplatesPage(query);
	}
