...
public class AttributesQuery extends HibernateQuery {
	...
	static private kbee.util.logging.Logger logger = kbee.util.logging.Logger.getLogger(AttributesQuery.class.getName());
	
	public AttributesQuery() {
		this(true);
	}
	
	public AttributesQuery(boolean deleted_visible) {
	...
	public String getStatement() {
		...
		statement.append("from KbeeAttribute R where R.domain.id=" + String.valueOf(domain.getId()) +" " + h_text.toString());
		
		String sizeQuery = "select count (*) FROM KbeeAttribute R WHERE R.domain.id=" + domain.getId().toString() +"  " + h_text.toString();
