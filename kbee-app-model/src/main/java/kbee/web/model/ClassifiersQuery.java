...
public class ClassifiersQuery extends HibernateQuery {
			
	static private kbee.util.logging.Logger logger = kbee.util.logging.Logger.getLogger(ClassifiersQuery.class.getName());
	boolean deleted_visible = true;
	
	private static final long serialVersionUID = 1L;
	
	public ClassifiersQuery() {
	}			
	
	public ClassifiersQuery(boolean deleted_visible) {
	...
	public String getStatement() {
		...
		statement.append("from KbeeClassifier R where R.domain.id=" + String.valueOf(domain.getId()) +"  " + h_text.toString());
		
		String sizeQuery = "select count (*) FROM KbeeClassifier R WHERE R.domain.id=" + domain.getId().toString() +" "+ h_text.toString();
		...
		else if ("dataset".equals(getParameters().get("sort"))) {
			statement.append(" order by lower(R.dataset1.name) " + str_order);
		}
