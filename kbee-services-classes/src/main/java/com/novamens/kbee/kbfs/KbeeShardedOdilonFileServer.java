...
public class KbeeShardedOdilonFileServer implements FileServerOdilon {
				
	static private Logger logger 		= LogManager.getLogger(KbeeShardedOdilonFileServer.class.getName());
	...
	private boolean isReadOnly = false;
 	...
 	private Map<Integer,  FileServerOdilon> f2_odilon_shard = new ConcurrentHashMap<Integer,  FileServerOdilon>();
 								
 	private Map<String, Integer> f2_odilon_fsid_shard = new ConcurrentHashMap<String, Integer>();
 	...
 	public KbeeShardedOdilonFileServer () throws FileServerException {
		startupLogger.info("Odilon Shard Manager");
		startupLogger.info(" Starting Sharded Odilon File Server");
	}
	...
	public void putObject(Integer shard, String bucketName, String objectName, String filename, InputStream stream,
			long size, String contentType) throws FileServerException {
	...
	public Map<Integer, FileServerOdilon> getShards() {
		if (!isInitialized()) {
			try {
				init();
			} catch (FileServerException e) {
	...
	/**
	 * 
	 * Minimum 1 MinioFileServer
	 * Max 6 MinioServers. Each of them can be Stand alone or Distributed  
	 * 
	 * @throws FileServerException
	 * 
	 */
	private synchronized void init() throws FileServerException {
		...
		synchronized (this) {
			...
			if (this._initialized)
	 	 	...
	 	 	String ACCESSKEY 	= PropertiesFactory.getInstance("kbee").getProperties().getProperty("odilon.accesskey",    	"odilon").trim();
	 	 	String READONLY		= PropertiesFactory.getInstance("kbee").getProperties().getProperty("odilon.readonly",    	"no").toLowerCase().trim();
	 	 	String SECRETKEY 	= PropertiesFactory.getInstance("kbee").getProperties().getProperty("odilon.secretkey",    	"odilon").trim();
	 	 	String FSID	 		= PropertiesFactory.getInstance("kbee").getProperties().getProperty("odilon.fsid", 	  	 	"fs-dev1").trim();
	 	 	String TYPE			= PropertiesFactory.getInstance("kbee").getProperties().getProperty("odilon.type", 			"odilon").trim();
	 	 		
	 	 	String URL1			= PropertiesFactory.getInstance("kbee").getProperties().getProperty("odilon.shard1.endpoint", 		URL).trim();
	 	 	String ACCESSKEY1 	= PropertiesFactory.getInstance("kbee").getProperties().getProperty("odilon.shard1.accesskey",    	ACCESSKEY).trim();
	 	 	String SECRETKEY1 	= PropertiesFactory.getInstance("kbee").getProperties().getProperty("odilon.shard1.secretkey",    	SECRETKEY).trim(); 	 	
	 	 	String READONLY1	= PropertiesFactory.getInstance("kbee").getProperties().getProperty("odilon.shard1.readonly",    	READONLY).toLowerCase().trim();
	 	 	String FSID1 		= PropertiesFactory.getInstance("kbee").getProperties().getProperty("odilon.shard1.fsid",    		FSID).trim();
	 	 	String TYPE1		= PropertiesFactory.getInstance("kbee").getProperties().getProperty("odilon.type", 					TYPE).toLowerCase().trim();
		 	
		 	try {
	
		 		if (TYPE1.equals("odilon")) {
		 			KbeeOdilonFileServer kbfs = new KbeeOdilonFileServer(URL1, ACCESSKEY1, SECRETKEY1, FSID1, Integer.valueOf(++var_shard_counter), 1.0);
			 	 	kbfs.setReadOnly(READONLY1.equals("yes") || READONLY1.equals("true"));
			 	 	this.f2_odilon_shard.put(Integer.valueOf(var_shard_counter), kbfs);
			 	 	this.f2_odilon_fsid_shard.put(kbfs.getFSId(), Integer.valueOf(var_shard_counter));
		 		}
	...
	private void add(int order) throws FileServerException {
		...
		final String _URL = PropertiesFactory.getInstance("kbee").getProperties().getProperty("odilon.shard"+String.valueOf(order).trim()+".endpoint", "null").trim();
 	 	
		if (!_URL.equals("null")) {
							
			final String _ACCESSKEY 		= PropertiesFactory.getInstance("kbee").getProperties().getProperty("odilon.shard"+String.valueOf(order).trim()+".accesskey", "odilon").trim();
 	 	 	final String _READONLY			= PropertiesFactory.getInstance("kbee").getProperties().getProperty("odilon.shard"+String.valueOf(order).trim()+".readonly",   "no").toLowerCase().trim();
 	 	 	final String _SECRETKEY	 		= PropertiesFactory.getInstance("kbee").getProperties().getProperty("odilon.shard"+String.valueOf(order).trim()+".secretkey", "odilon").trim();
 	 	 	final String _FSID				= PropertiesFactory.getInstance("kbee").getProperties().getProperty("odilon.shard"+String.valueOf(order).trim()+".fsid",    "fs-dev"+String.valueOf(order)).trim();
 	 	 	final String _TYPE				= PropertiesFactory.getInstance("kbee").getProperties().getProperty("odilon.shard"+String.valueOf(order).trim()+".type",    "odilon").trim();
 	 	 	final String _MINOR				= PropertiesFactory.getInstance("kbee").getProperties().getProperty("odilon.shard"+String.valueOf(order).trim()+".minor",    "no").trim();
 	 	 	
 	 	 	if (_TYPE.equals("odilon")) {
	 	 	 	KbeeOdilonFileServer fs = new KbeeOdilonFileServer(_URL, _ACCESSKEY, _SECRETKEY,  _FSID, Integer.valueOf(++var_shard_counter));
	 	 	 	fs.setMinor(_MINOR.equals("yes"));
	 	 	 	String pg = fs.ping();
	 	 		if (pg.equals("ok")) {
		 	 		fs.setReadOnly(_READONLY.equals("yes") || _READONLY.equals("true"));
		 	 	 	this.f2_odilon_shard.put(Integer.valueOf(var_shard_counter), fs);
		 	 	 	this.f2_odilon_fsid_shard.put(this.f2_odilon_shard.get(Integer.valueOf(var_shard_counter)).getFSId(), Integer.valueOf(var_shard_counter));
	 	 		}
	 	 		else {
	 	 			startupLogger.error("Odilon Shard " + order +" - " + _URL + " | Ping: " +  pg );
	 	 			throw new FileServerException("Odilon Shard " + order +" - " + _URL + " | Ping: " +  pg );
	 	 		}
	 	 		startupLogger.info("Odilon Shard Manager. Added Shard_" + String.valueOf(order) + " -> " + _URL);
	 	 		
 	 	 	}
		} else {
			final String _ACCESSKEY = PropertiesFactory.getInstance("kbee").getProperties().getProperty("odilon.shard"+String.valueOf(order).trim()+".accesskey", "").trim();
			if (_ACCESSKEY!=null && _ACCESSKEY.length()>0)
				startupLogger.error("There is an AccessKey without endpoint -> " + _ACCESSKEY + " \n Please check configuration file");
		}
	}
	...
	private void setProbabilities() {
		Double total_rw= Double.valueOf(0.0);
 	 	boolean found = false;
 	 	for (Entry<Integer,  FileServerOdilon> entry: this.f2_odilon_shard.entrySet()) {
	 	...
	 	for (int t=0; t<this.f2_odilon_shard.size(); t++) {
	 		String pbs;
	 		
	 		if (t==0) {
	 			pbs=PropertiesFactory.getInstance("kbee").getProperties().getProperty("odilon.probability", "null").trim();
	 			if (pbs.equals("null")) 
	 				pbs=PropertiesFactory.getInstance("kbee").getProperties().getProperty("odilon.shard"+String.valueOf(t+1).trim()+".probability", String.valueOf(pbase)).trim();
	 		}
	 		else
	 			pbs=PropertiesFactory.getInstance("kbee").getProperties().getProperty("odilon.shard"+String.valueOf(t+1).trim()+".probability", String.valueOf(pbase)).trim();
 	 		...
 	 		try {
 	 			...
 	 			if (this.f2_odilon_shard.containsKey(Integer.valueOf(t+1)) && this.f2_odilon_shard.get(t+1).isReadOnly()) 
 	 		...
 	 		if (this.f2_odilon_shard.containsKey(Integer.valueOf(t+1))) {
 	 			this.f2_odilon_shard.get(Integer.valueOf(t+1)).setProbability(pb);
 	 		}
 	 		ptotal += pb;
 	 	}
	 	...
	 	if (ptotal>1.00 || ptotal<0.99) {
	 		if (ptotal<0.1) {
	 			...
	 			this.f2_odilon_shard.get(1).setProbability(1.0);
	 		}
	 		else if (ptotal>1.00) {
	 			if (total_rw>0) {
			 		...
			 		for(int n=0; n<this.shard_probability.size(); n++) {
			 				this.shard_probability.set(n, this.shard_probability.get(n)*normalize_factor);
				 	 		if (this.f2_odilon_shard.containsKey(Integer.valueOf(n+1))) {
				 	 			this.f2_odilon_shard.get(Integer.valueOf(n+1)).setProbability(this.shard_probability.get(n));
				 	 		}
			 		}
	 			}
	 		}
	 			...
	 			if (total_rw>0) {
		 			...
		 			for(int n=0; n<this.shard_probability.size(); n++) {
		 				if (this.f2_odilon_shard.containsKey(Integer.valueOf(n+1)) && !(this.f2_odilon_shard.get(Integer.valueOf(n+1)).isReadOnly())) {
	 	...
	 	startupLogger.info("Odilon Shard Manager");
    	startupLogger.info("Total Odilon Servers ->" + String.valueOf(total_shards));
    	
    	int k=1;
    	
    	for (Double d: this.shard_probability) {
    		startupLogger.info("Odilon Shard Manager. Shard_" + String.valueOf(k) + ". Probability  -> " + String.valueOf(d));
    		k++;
    	}
	}
