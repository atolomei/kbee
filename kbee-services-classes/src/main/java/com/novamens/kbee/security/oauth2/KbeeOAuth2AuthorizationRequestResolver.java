 ...
 */
public final class KbeeOAuth2AuthorizationRequestResolver implements OAuth2AuthorizationRequestResolver {
	 ...
	 */
	public KbeeOAuth2AuthorizationRequestResolver(ClientRegistrationRepository clientRegistrationRepository,
	...
	private static String expandRedirectUri(HttpServletRequest request, ClientRegistration clientRegistration, String action) {
		...
		uriVariables.put("baseScheme", scheme == null ? "" : ("localhost".equals(host) ? scheme : "https"));
		...
		String uri = UriComponentsBuilder.fromUriString(clientRegistration.getRedirectUriTemplate())
				.buildAndExpand(uriVariables)
				.toUriString();
		
		if (!"localhost".equals(host)) uri = uri.replace("http:","https:");
