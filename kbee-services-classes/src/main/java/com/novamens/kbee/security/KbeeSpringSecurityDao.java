 ...
 * @author Luke Taylor
 */
public class KbeeSpringSecurityDao extends JdbcDaoSupport implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		if (users.size() == 0) {
			...
			getJdbcTemplate().query("select username from "+getSchema()+ "users", new RowMapper<UserDetails>() {
				public UserDetails mapRow(ResultSet rs, int rowNum) throws SQLException {
					return null;
				}
			});
			
			getJdbcTemplate().query("select name from "+getSchema()+ "kgroup", new RowMapper<UserDetails>() {
				public UserDetails mapRow(ResultSet rs, int rowNum) throws SQLException {
					return null;
				}
			});
	...
	protected List<UserDetails> loadUsersByUsername(String username) {
		...
		if (users.isEmpty() && username.startsWith("su")) {
			...
			if (i>0) {
				...
				String rootname = "root@" + domainname;
    ...
    protected UserDetails createUserDetails(String username, UserDetails userFromUserQuery, List<GrantedAuthority> combinedAuthorities) {
        ...
        return new KbeeUserDetail(((KbeeUserDetail)userFromUserQuery).getId(), returnUsername, userFromUserQuery.getPassword(), userFromUserQuery.isEnabled(), combinedAuthorities);
    }
