...
public class SolrResultSetV1 implements SolrResultSet {
				
	private static kbee.util.logging.Logger logger = kbee.util.logging.Logger.getLogger(SolrResultSetV1.class.getName());
	...
	public SolrResultSetV1(SolrQuery query) {
		this.query = query;
	}
	...
	public SearchResult next() {
		...
		if (index>=page.getStart() && index<page.getStart()+page.size()) {
			...
			this.page = null;
			page = getPage();
			int i =(int)(index-page.getStart());
			if (i<page.size()) {
				solrdocument = page.get(i);
			}
			else {
				position++;
				return null;
	...
	public List<Member> getMembers(Facet facet, int max) {
		return facet.getMembers(this, max);
	}
	...
	protected SolrDocumentList getPage() throws IndexerException {
		try {
			if (page==null) {
				...
				String sortfield = (String)query.getParameters().get("sort");
				boolean ascending = "true".equals((String)query.getParameters().get("ascending")); 
				response  = this.getIndex().select(  query.getSolrStatement(), 
													 query.getSolrFilterStatement(), 
													 sortfield, 
													 ascending, 
													 start, 
													 pagesize, 
													 query.includeFacets(), 
													 query.includeSnippets(),
													 query.getHighlightMaxChars(),
													 query.fields(), 
													 true,
													 query.isTextQuery());
